...
The easiest way to get started with Smooks is to download and try out the https://github.com/smooks/smooks-examples/tree/v2[examples]. The examples are the recommended base upon which to integrate Smooks into your application.
// end::getting-started[]
...
Comparing the https://github.com/smooks/smooks/tree/v1.7.1/smooks-examples[code examples] for Smooks 1 with https://github.com/smooks/smooks-examples/tree/v2[those for Smooks 2] can be a useful guide in migrating to Smooks 2. While not exhaustive, we have compiled a list of notes to assist your migration:
...
One or more of https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/resource/visitor/sax/ng/SaxNgVisitor.html[SaxNgVisitor] interfaces need to be implemented in order to consume the SAX event stream produced from the source, depending on which events are of interest.
...
Observe that in this case the program does not produce a result. The program does not even interact with the filtering process in any way because it does not provide an https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/ExecutionContext.html[`+ExecutionContext+`] to https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/Smooks.html[`+smooks.filterSource(...)+`].
...
. As instances of https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/io/Sink.html[`+Sink+`]: client code extracts output from the `+Sink+` instance after passing an empty one to `+Smooks#filterSource(...)+`.
...
Smooks can work with implementation of https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/io/sink/StreamSink.html[`+StreamSink+`] and https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/io/sink/DOMSink.html[`+DOMSink+`] sink types, as well as:

* https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/io/sink/JavaSink.html[`+JavaSink+`]: sink type for capturing the contents of the Smooks JavaBean context.

* https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/io/sink/StringSink.html[`+StringSink+`]: `+StringSink+` extension wrapping a `+StringWriter+`, useful for testing.
...
Of course you can also write and use your own https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/delivery/event/ExecutionEventListener.html[ExecutionEventListener] implementations.
...
* https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/engine/bean/context/preinstalled/UniqueID.html[`+PUUID+`]: This `+UniqueId+` instance provides unique identifiers for the filtering `+ExecutionContext+`.

* https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/engine/bean/context/preinstalled/Time.html[`+PTIME+`]: This `+Time+` instance provides time-based data for the filtering ExecutionContext.
...
* Global Configuration Parameters: Every in a Smooks configuration can specify elements for configuration parameters. These parameter values are available at runtime through the https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/resource/config/ResourceConfig.html[`+ResourceConfig+`], or are reflectively injected through the `+@Inject+` annotation. Global Configuration Parameters are parameters that are defined centrally (see below) and are accessible to all runtime components via the `+ExecutionContext+` (vs `+ResourceConfig+`). More on this in the following sections.
...
The CSV and Regex readers are implemented using these abstract classes. See the https://github.com/smooks/smooks-examples/tree/v2/csv-variable-record[csv-variable-record] and https://github.com/smooks/smooks-examples/tree/v2/flatfile-to-xml-regex[flatfile-to-xml-regex] examples. The https://github.com/smooks/smooks-examples/tree/v2/flatfile-to-xml-regex[Regex Reader] implementation is also a good example that can be used as a basis for your own custom flat file reader.
...
https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/resource/visitor/sax/ng/BeforeVisitor.html[`+BeforeVisitor+`]: Captures the _startElement_ SAX event for the targeted fragment element:
...
https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/resource/visitor/sax/ng/AfterVisitor.html[`+AfterVisitor+`]: Captures the _endElement_ SAX event for the targeted fragment element:
...
As a convenience for those implementations that need to capture all the SAX events, the above three interfaces are pulled together into a single interface in the https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/api/resource/visitor/sax/ng/ElementVisitor.html[`+ElementVisitor+`] interface.
...
So, if the Source provided to the `+Smooks.filterSource(Source, Sink)+` method is an XML stream and a  https://www.smooks.org/javadoc/v2.0.0/smooks/org/smooks/io/StreamSink.html[`+StreamSink+`] instance is provided as one of the `+Sink+` instances, the Source XML will be written out to the
