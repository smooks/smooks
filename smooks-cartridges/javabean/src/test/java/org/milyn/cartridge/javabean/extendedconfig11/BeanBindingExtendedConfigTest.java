...
*/
package org.milyn.cartridge.javabean.extendedconfig11;

import com.google.common.io.Resources;
import junit.framework.Assert;
import junit.framework.TestCase;
import org.milyn.Smooks;
import org.milyn.cartridge.javabean.B;
import org.milyn.cartridge.javabean.Header;
import org.milyn.cartridge.javabean.OrderItem;
...
public class BeanBindingExtendedConfigTest extends TestCase {
    ...
    private void assertOrderOK(ExtendedOrder order, boolean checkArrays) {
        ...
        Assert.assertEquals(new Long(123123), order.getHeader().getCustomerNumber());
        Assert.assertEquals(1163616328000L, order.getHeader().getDate().getTime());
        Assert.assertEquals(true, order.getHeader().getPrivatePerson());
        ...
        Assert.assertEquals(8.9d, order.getOrderItems().get(0).getPrice());
        Assert.assertEquals(111, order.getOrderItems().get(0).getProductId());
        Assert.assertEquals(new Integer(2), order.getOrderItems().get(0).getQuantity());
        assertTrue(order == order.getOrderItems().get(1).getOrder());
        Assert.assertEquals(5.2d, order.getOrderItems().get(1).getPrice());
        Assert.assertEquals(222, order.getOrderItems().get(1).getProductId());
        Assert.assertEquals(new Integer(7), order.getOrderItems().get(1).getQuantity());

        if (checkArrays) {
            // OrderItems array...
            Assert.assertEquals(2, order.getOrderItemsArray().length);
            assertTrue(order == order.getOrderItemsArray()[0].getOrder());
            Assert.assertEquals(8.9d, order.getOrderItemsArray()[0].getPrice());
            Assert.assertEquals(111, order.getOrderItemsArray()[0].getProductId());
            Assert.assertEquals(new Integer(2), order.getOrderItemsArray()[0].getQuantity());
            assertTrue(order == order.getOrderItemsArray()[1].getOrder());
            Assert.assertEquals(5.2d, order.getOrderItemsArray()[1].getPrice());
            Assert.assertEquals(222, order.getOrderItemsArray()[1].getProductId());
            Assert.assertEquals(new Integer(7), order.getOrderItemsArray()[1].getQuantity());
        }
    }
    ...
    public void test_error_for_no_property_on_non_list_or_array() throws IOException, SAXException {
        try {
            ...
            assertEquals("'wiring' binding for bean class 'org.milyn.cartridge.javabean.extendedconfig.ExtendedOrder' must specify a 'property' or 'setterMethod' attribute.", e.getCause().getMessage());
        }
    }
