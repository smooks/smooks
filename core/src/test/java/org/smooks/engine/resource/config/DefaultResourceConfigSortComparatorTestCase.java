...
public class DefaultResourceConfigSortComparatorTestCase {
	...
	public void test_getSpecificity_selector() {
		...
		//    0 -> Null namespace
        //    5 -> "*".
		assertSpecificityOK(10.0, "*", "*");

		//  110 -> Explicit contextual selector - 2 deep
		//    0 -> Null namespace
        //    5 -> "*".
		assertSpecificityOK(115.0, "table/tr", "*");

		//  110 -> Explicit contextual selector - 3 deep
		//    0 -> Null namespace
        //    5 -> "*".
		assertSpecificityOK(125.0, "table/tr/td", "*");
	}
	...
	public void test_compare() {
		...
		config1 = new DefaultResourceConfig("selector", new Properties(), "uaCommonName", null);
		assertEquals(0, sortComparator.compare(config1, config1));

		// 0	-> 2 configs of equal specificity
		config1 = new DefaultResourceConfig("selector", new Properties(), "uaCommonName", null);
		config2 = new DefaultResourceConfig("selector", new Properties(), "uaCommonName", null);
		assertEquals(0, sortComparator.compare(config1, config2));

		// -1	-> config1 more specific than config2
		config1 = new DefaultResourceConfig("selector", new Properties(), "uaCommonName", null);
		config2 = new DefaultResourceConfig("selector", new Properties(), "profile1", null);
		assertEquals(-1, sortComparator.compare(config1, config2));

		// 1	-> config2 more specific than config1
		config1 = new DefaultResourceConfig("selector", new Properties(), "profile1", null);
		config2 = new DefaultResourceConfig("selector", new Properties(), "uaCommonName", null);
		assertEquals(1, sortComparator.compare(config1, config2));
	}
	
	private void assertSpecificityOK(double expected, String selector, String targetProfile) {
		DefaultResourceConfigSortComparator sortComparator = new DefaultResourceConfigSortComparator(profileSet);
		ResourceConfig resourceConfig;
		double specificity;

		resourceConfig = new DefaultResourceConfig(selector, new Properties(), targetProfile, null);
