...
public class TypeConverterFactoryLoader {
    ...
    private void assertTypeConverterFactoryInterface(TypeConverterFactory<? , ?> typeConverterFactory) {
        ...
        if (genericInterfaces.length >= 1) {
            ...
            if (!assertion) {
                throw new SmooksConfigurationException(String.format("%s must immediately extend org.smooks.converter.factory.TypeConverterFactory", typeConverterFactory.getClass()));
            }
        } else {
            throw new SmooksConfigurationException(String.format("%s must extend org.smooks.converter.factory.TypeConverterFactory", typeConverterFactory.getClass()));
        }
    }
}
