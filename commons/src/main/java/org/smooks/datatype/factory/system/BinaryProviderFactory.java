 ...
 * @author bardl
 */
public class BinaryProviderFactory implements DataTypeProviderFactory<String, String> {

    private static final Pattern BINARY_PATTERN = Pattern.compile("^[01]+$");

    @Override
    public Provider<String> createProvider(final String value) {
        return () -> {
            final Matcher binaryMatcher = BINARY_PATTERN.matcher(value);
            if (binaryMatcher.matches()) {
                return value;
            } else {
                throw new DataDecodeException("Failed to decode binary sequence '" + value + "'.");
            }
        };
    }
}
