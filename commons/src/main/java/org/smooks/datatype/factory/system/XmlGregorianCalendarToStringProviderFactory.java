 ...
 */
public class XmlGregorianCalendarToStringProviderFactory implements DataTypeProviderFactory<XMLGregorianCalendar, String>, Configurable {

    private Properties config;

    @Override
    public Provider<String> createProvider(final XMLGregorianCalendar value) {
        LocaleAwareDateToStringProviderFactory<Date> localeAwareDateToStringProviderFactory = new LocaleAwareDateToStringProviderFactory<Date>() {
            @Override
            protected Provider<String> doCreateProvider(final String date) {
                return () -> date;
            }
        };
        
        if (config != null) {
            localeAwareDateToStringProviderFactory.setConfiguration(config);
        }
        
        return localeAwareDateToStringProviderFactory.createProvider(value.toGregorianCalendar().getTime());
    }
